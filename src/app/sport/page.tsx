...
export default function SportPage() {
  ...
  const sportItems = sortedCards.filter((item) => hasArticleCategory(item, "sport"));
  const featureStory = sportItems[0] ? articlesMap[sportItems[0].slug] : null;
  const gridItems = featureStory
    ? sportItems.filter((item) => item.slug !== featureStory.slug)
    : sportItems;
      ...
      <main className="flex-1">
        <section className="mx-auto max-w-[1116px] px-4 py-16 md:px-6 md:py-20">
          ...
          <p className="mx-auto mt-4 max-w-[760px] text-center text-[17px] font-body font-normal leading-[1.7] text-[rgba(0,0,0,0.7)]">
            From athletes shaping culture to the moments that move the crowd, Sport brings together stories where performance, influence, and visibility meet.
          </p>

          {featureStory ? (
            <div className="mt-14 grid grid-cols-1 gap-8 lg:grid-cols-[563px_minmax(0,1fr)] lg:items-start">
              <Link
                href={`/articles/${featureStory.slug}?from=sport`}
              ...
              <div className="max-w-[370px] pt-1">
                ...
                <Link href={`/articles/${featureStory.slug}?from=sport`} className="block">
          ...
          {visibleItems.length ? (
            <div className="mt-14 grid grid-cols-1 gap-x-[28px] gap-y-12 sm:grid-cols-2 xl:grid-cols-3">
              {visibleItems.map((item, index) => (
                <article key={`${item.slug}-${index}`} className="max-w-[321px]">
                  <Link
                    href={`/articles/${item.slug}?from=sport`}
                    className="group block transition-transform duration-300 hover:-translate-y-1"
                  >
            ...
            <div className="mt-14 rounded-[20px] border border-black/8 bg-[#faf8f6] px-6 py-10 text-center">
              <h2 className="text-[24px] font-body font-bold tracking-[-0.03em] text-[#181818]">
                No sport stories are live yet
              </h2>
              <p className="mx-auto mt-3 max-w-[560px] text-[15px] font-body font-normal leading-[1.7] text-[rgba(0,0,0,0.64)]">
                As soon as the first sport article is published, it will appear here automatically with the same layout as the rest of the website.
              </p>
            </div>
          )}
