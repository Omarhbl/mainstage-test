...
import { getPublicArticlesBySlug, getSortedPublicArticleCards } from "@/lib/public-articles";

export const dynamic = "force-dynamic";

export default async function CinemaArticlesPage() {
  const cinemaArticles = (await getSortedPublicArticleCards()).filter((article) =>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-[980px] whitespace-nowrap text-[18px] font-body font-normal leading-[1.65] text-[rgba(0,0,0,0.68)]">
                Explore every cinema story published on Mainstage, from big-screen
                comebacks to what&apos;s shaping the next release cycle.
              </p>
            </div>
          </div>
          ...
          <div className="mt-14 grid grid-cols-1 gap-x-[28px] gap-y-12 sm:grid-cols-2 xl:grid-cols-3">
            {cinemaArticles.map((article) => (
              <article key={article.slug} className="max-w-[321px]">
                <Link href={`/articles/${article.slug}?from=cinema`} className="block">
